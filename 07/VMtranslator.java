...
public final class VMtranslator {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String asmFilePath = file.toString().substring(0, file.toString().lastIndexOf(DOT)) +
					ASM_EXTENSION;
			Vector<VMparser> parsers = new Vector<>();
			if (file.isFile()) parsers.add(new VMparser(file));
			else if (file.isDirectory()) {
				for (File fileEntry : file.listFiles()) {
					if (file.getName().endsWith(VM_EXTENSION)) {
			...
			for (VMparser VMparser : parsers) {
				...
				while (!VMparser.hasFinished()) {
					switch (VMparser.getCmdType()) {
						case C_ARITHMETIC:
							asmWriter.writeArithmetic(VMparser.getCurrentCommand());
							break;
						case C_PUSH:
